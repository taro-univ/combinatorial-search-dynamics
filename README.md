...
uv run csd doctor
uv run csd validate-repository
uv run dvc dag
```
...
```bash
uv run csd validate-data --data-dir /path/to/dataset
```

`--data-dir`を省略するとHydraの`storage.data_dir`を使用します。対象が存在しない、または空の場合は非0で終了します。空を意図的に許容する構成確認だけを行う場合は`--allow-empty`を明示してください。

```bash
uv run csd validate-data --data-dir /path/to/empty-directory --allow-empty
```
...
```bash
uv run csd generate-instances
uv run csd solve-references
uv run csd collect
uv run csd prepare
uv run csd extract
uv run csd split
uv run csd analyze-success
uv run csd fit
uv run csd evaluate
uv run csd report
```
...
```bash
uv run csd reproduce-pilot experiment=knapsack_pilot task=knapsack solver=ortools_cp_sat \
  state_model=objective_gap experiment.instance_count=12 task.item_count=6 search.trials=2 \
  storage.raw_data_dir=/tmp/csd-pilot/raw storage.interim_data_dir=/tmp/csd-pilot/interim \
  storage.derived_data_dir=/tmp/csd-pilot/derived storage.artifact_dir=/tmp/csd-pilot/artifacts \
  tracking.tracking_uri=file:/tmp/csd-pilot/mlruns
```
...
```bash
uv run csd generate-instances experiment=knapsack_pilot task=knapsack
uv run csd solve-references experiment=knapsack_pilot task=knapsack solver=ortools_cp_sat
uv run csd reproduce-pilot experiment=knapsack_pilot task=knapsack solver=ortools_cp_sat state_model=objective_gap
```

Phase 2のdummy回帰は別pathを使います。既存出力がある場合は明示的に`--force`を使うか、隔離pathを指定します。

```bash
uv run csd reproduce-pilot experiment=pilot task=dummy_binary solver=none state_model=baseline storage=local tracking=local
```
...
Codex CLIはリポジトリ開発を補助する外部ツールであり、研究対象LLMのadapterではありません。必要な開発者だけが別途導入し、`./scripts/run_codex.sh`または`csd codex -- <prompt>`から利用できます。Phase 0のセットアップとCIには含まれません。

## 構成

- `configs/`: Hydra設定の正本
- `src/combinatorial_search_dynamics/identifiers.py`: canonical JSONによるID生成
- `src/combinatorial_search_dynamics/data/`: schema、Parquet、Zarr、DuckDB、検証
